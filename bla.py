...
try:
	...
	paths = ["general/create.sql",
			 "general/insert_Users.sql", "general/insert_Meetups.sql", "general/insert_MeetupRegistrations.sql"]
	...
	content = " ".join("""
	-- listing meetups after 2015.11.27
	SELECT Start, Location, Topic FROM Meetups
	WHERE Start > "2015-11-27";

	-- listing users who have introduction
	SELECT Name, Birthdate, Email FROM Users
	WHERE Introduction LIKE "%";

	-- listing all meetup registrations for a particular user
	SELECT Id, StatusId, MeetupId, UserId FROM MeetupRegistrations
	WHERE UserId="5";
	""".split('\n'))

	print("Executing querry commands not included into sql files:")
	raw_commands = content.split(";")
	clear_commands = [cmd.strip() for cmd in raw_commands]
	for command in clear_commands:
		try:
